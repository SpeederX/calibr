...
for (const md of MODELS) {
  for (const ctx of CTXES) {
    ...
    DATA.push({
      ...
      ttfh_ms:           hasExt ? 8 : null,
      stream_open_ms:    hasExt ? 14 : null,
      client_ttft_ms:    hasExt ? Math.round(300 + 20 * ctxRatio) : null,
      server_prefill_ms: hasExt ? Math.round(250 + 18 * ctxRatio) : null,
      server_ttft_ms:    hasExt ? Math.round(280 + 20 * ctxRatio) : null,
      tpot_ms:           hasExt ? +(1000 / tps).toFixed(2) : null,
      itl_p95_ms:        hasExt ? +(1250 / tps).toFixed(2) : null,
      e2e_latency_ms:    hasExt ? Math.round(2200 + 80 * ctxRatio) : null,
      delivery_gap_p95_ms: hasExt ? +(1300 / tps).toFixed(2) : null,
      ...
      runs: hasExt ? [{ run_index:1, telemetry:[
        { elapsed_ms:0, phase:"warmup", vram_total_mib:baseline, vram_run_mib:0, ram_used_mib:0, shared_mib:0, gpu_util_pct:2, cpu_util_pct:4 },
        { elapsed_ms:450, phase:"throughput", vram_total_mib:vram + baseline - 120, vram_run_mib:vram - 120, ram_used_mib:500, shared_mib:0, gpu_util_pct:94, cpu_util_pct:18 },
        { elapsed_ms:900, phase:"latency_prompt", event_index:0, event_kind:"prefill", prompt_processed:1024, prompt_total:4096, prompt_time_ms:70, vram_total_mib:vram + baseline, vram_run_mib:vram, ram_used_mib:700, shared_mib:shared, gpu_util_pct:96, cpu_util_pct:22 },
        { elapsed_ms:1010, phase:"latency_prompt", event_index:1, event_kind:"prefill", prompt_processed:4096, prompt_total:4096, prompt_time_ms:180, vram_total_mib:vram + baseline, vram_run_mib:vram, ram_used_mib:715, shared_mib:shared, gpu_util_pct:97, cpu_util_pct:20 },
        { elapsed_ms:1080, phase:"latency_reasoning", event_index:2, event_kind:"reasoning", server_predicted_n:1, server_predicted_ms:20, server_tps:null, delivery_gap_ms:null, vram_total_mib:vram + baseline, vram_run_mib:vram, ram_used_mib:720, shared_mib:shared, gpu_util_pct:97, cpu_util_pct:20 },
        { elapsed_ms:1120, phase:"latency_reasoning", event_index:3, event_kind:"reasoning", server_predicted_n:3, server_predicted_ms:55, server_tps:tps * 1.12, delivery_gap_ms:40, vram_total_mib:vram + baseline, vram_run_mib:vram, ram_used_mib:725, shared_mib:shared, gpu_util_pct:98, cpu_util_pct:20 },
        { elapsed_ms:1180, phase:"latency_answer", event_index:4, event_kind:"answer", server_predicted_n:6, server_predicted_ms:110, server_tps:tps * 0.96, delivery_gap_ms:60, vram_total_mib:vram + baseline, vram_run_mib:vram, ram_used_mib:730, shared_mib:shared, gpu_util_pct:98, cpu_util_pct:20 },
        { elapsed_ms:1260, phase:"latency_answer", event_index:5, event_kind:"answer", server_predicted_n:10, server_predicted_ms:190, server_tps:tps * 0.72, delivery_gap_ms:80, vram_total_mib:vram + baseline, vram_run_mib:vram, ram_used_mib:735, shared_mib:shared, gpu_util_pct:95, cpu_util_pct:23 }
      ]}] : [],
